...
export class Frier {
    public static blurFilterId = 'blurFilterId';
    public static convFilterId = 'convFilterId';
...
class FriedMeme {
    ...
    private async _filterImage(
        ...
        this.ctx.filter = ''
            ...
            + (this.useSharpness ? `url("#${Frier.convFilterId}") ` : '')
            + `brightness(${this.brightness}) 
               contrast(${this.contrast}) 
               hue-rotate(${this.hueRotate}deg) `
            + (this.blurStdDeviation > 0 ? `url("#${Frier.blurFilterId}") ` : '')
            ;
        ...
        ctx2.filter = `url("#${Frier.convFilterId}")  `;
