...
export class MemeFrier extends PolymerElement {
  ...
  public brightness = 2;
  public contrast = 4;
  public lensWidth = 300;
  public lensHeight = 150;
  ...
  public connectedCallback() {
    ...
    this.globalCompositeOperationIndex = this.globalCompositeOperations.indexOf(
      this.globalCompositeOperation
    );

    this.$.globalCompositeOperations.addEventListener(
      "iron-select",
      (e: Event) => {
    ...
    this.$.drawer.addEventListener("click", (e: Event) => {
      if (!((this.$.drawer as unknown) as AppDrawerElement).persistent) {
        ...
        if (
          (e.target as HTMLElement).id.toString() !== "globalCompositeOperation"
        ) {
          ((this.$.drawer as unknown) as AppDrawerElement).close();
        }
      }
    ...
    this.$.chooseFile.addEventListener(
      "change",
      (e: Event) => {
        if (
          e &&
    ...
    afterNextRender(this, () => {
      ...
      if (((this.$.drawer as unknown) as AppDrawerElement).persistent) {
        ...
        //     (this.$.chooseFile as HTMLElement).click()
        // );
        this.$["float-right-icon"].addEventListener("click", e =>
          (this.$.chooseFile as HTMLElement).click()
        );
        ...
        this.$.meme.addEventListener(
          "drop",
          e => {
            // (e: DragEvent) produces type error
            console.log("drop", e);
  ...
  static get properties() {
    return {
      ...
      lensWidth: { type: Number, reflectToAttribute: true, notify: true },
      lensHeight: { type: Number, reflectToAttribute: true, notify: true },
